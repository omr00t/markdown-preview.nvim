...
get_latest_release() {
  fetch "https://api.github.com/repos/omr00t/markdown-preview.nvim/releases/latest" |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/'
}
...
download() {
  mkdir -p bin
  cd bin
  url="https://github.com/omr00t/markdown-preview.nvim/releases/download/$tag/${1}"
